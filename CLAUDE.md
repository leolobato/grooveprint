...
Grooveprint is a local "Shazam for vinyl" — it identifies vinyl records playing on a turntable by matching microphone audio against a personal music catalog. Three components: a Python/FastAPI matching server, ingestion scripts, and client apps (Android tablet + legacy macOS).
...
- `js/app.js` — `grooveprintApp()` Alpine data function with SSE connection, CRUD, file upload
- `css/style.css` — Dark theme, responsive at 768px breakpoint

The now-playing view connects to `/now-playing/stream` (SSE) for real-time updates. Three states: idle, listening, playing. The web UI detects the Android client via `Grooveprint-Android` user agent string and shows additional controls (listen toggle, logout).
...
**WebView bridge:** The WebView injects a `Grooveprint` JavaScript interface with `startListening()`, `stopListening()`, `openSettings()`, `isListening()` methods. Custom user agent includes `Grooveprint-Android/1.0`.
