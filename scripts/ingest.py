#!/usr/bin/env python3
"""Ingestion script: reads audio files and feeds them to the grooveprint server."""
    ...
    def fetch_discogs_tracklist(discogs_url):
        ...
        try:
            r = requests.get(
                ...
                headers={"User-Agent": "Grooveprint/1.0"}, timeout=15,
            )
            r.raise_for_status()
            data = r.json()
...
DEFAULT_CONFIG_PATH = Path.home() / ".grooveprint.toml"
...
def fetch_discogs_cover(discogs_url: str, album_folder: Path) -> tuple[Path, str] | None:
    ...
    _discogs_throttle()
    try:
        img_r = requests.get(primary["uri"], headers={"User-Agent": "Grooveprint/1.0"}, timeout=30)
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest audio files into the grooveprint fingerprint server."
    )
