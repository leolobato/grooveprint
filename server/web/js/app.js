function grooveprintApp() {
  return {
    ...
    // Android client
    isAndroidClient: /Grooveprint-Android/.test(navigator.userAgent),
    ...
    toggleListening() {
      if (!window.Grooveprint) return;
      if (this.clientListening) {
        window.Grooveprint.stopListening();
        this.clientListening = false;
        this.nowPlaying = { status: 'idle' };
      } else {
        window.Grooveprint.startListening();
    ...
    openAppSettings() {
      if (window.Grooveprint) {
        window.Grooveprint.openSettings();
      }
    },
